...
function createInstance() {
	...
	function multiGet(keys) {
		return AsyncStorage.multiGet(keys).then(pairs => {
			const inFS = pairs.filter(([k, v]) => v === PLACEHOLDER).map(pair => pair[0]);
	...
	function multiSet(pairs) {
		...
		for (var i = 0; i < pairs.length; i++) {
			const pair = pairs[i];
			if ((pair[1].length > exports.THRESHOLD_SIZE)) {
				inFileSystem.push([pair[0], pair[1]]);
				pair[1] = PLACEHOLDER;
			}
		}

		return Promise.all([AsyncStorage.multiSet(pairs), fsStorage.multiSet(inFileSystem)]);
	}
